 ...
 *
 * Note: migration 0033 in this repo still declares the older
 * `RETURNS JSONB` form. The repaired definition reached the database through
 * KAN-62 without a migration on this branch's history — see the report.
 */
...
export function emptyPublishedFormSchema(): PublishedFormSchema {
  return { id: null, version: null, publishedAt: null, fields: [] }
}
...
export async function loadPublishedFormSchema(
  ...
  return {
    ...
    fields: normalizeFields(row.schema_json),
  }
}
